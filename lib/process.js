...
function ast_walker(ast) {
        ...
        var walkers = {
                ...
                "dot": function(expr) {
                        return [ "dot", walk(expr) ].concat($C.slice(arguments, 1));
                },
                ...
                "seq": function() {
                        return [ "seq" ].concat($C.slice(arguments).map(walk));
                },
...
function ast_squeeze(ast, options) {
        ...
        function stats_to_sequences(statements) {
                for (var i = 0, ret = [], prev = null; i < statements.length; ++i) {
                        var st = statements[i];
                        if (!prev) {
                                if (st[0] == "stat") {
                                        prev = [ "seq", st[1] ];
                                ...
                                prev.push.apply(prev, $C.slice(st, 1));
                        } else {
                                prev = null;
                                ret.push(st);
                        }
        ...
        function join_consecutive_vars(statements) {
                for (var i = 0, ret = [], prev = null; i < statements.length; ++i) {
                        var st = statements[i];
                        if (prev && ((st[0] == "var" && prev[0] == "var") ||
                                     (st[0] == "const" && prev[0] == "const"))) {
                                prev[1] = prev[1].concat(st[1]);
                        } else {
                                ret.push(st);
                                prev = st;
                        }
        ...
        function discard_unnecessary_blocks(statements) {
                for (var i = 0, ret = []; i < statements.length; ++i) {
                        var st = statements[i];
                        if (st[0] == "block") {
                                if (st[1]) {
                                        ret.push.apply(ret, st[1]);
                                }
                        } else {
                                ret.push(st);
                        }
        ...
        function tighten(statements) {
                statements = statements.map(walk);
                statements = join_consecutive_vars(statements);
                statements = discard_unnecessary_blocks(statements);
                if (options.make_seqs)
                        statements = stats_to_sequences(statements);
        ...
        function _lambda(name, args, body) {
                return [ this[0], name, args, tighten(body) ];
        };
        ...
        return w.with_walkers({
                ...
                "switch": function(expr, body) {
                        ...
                        return [ "switch", walk(expr), body.map(function(branch, i){
                                var ca$e = branch[0] ? walk(branch[0]) : null;
...
var DOT_CALL_NO_PARENS = $C.array_to_hash([
        "name",
        "array",
        "string",
        "dot",
...
function gen_code(ast, beautify) {
        if (beautify) beautify = $C.defaults(beautify, {
                indent_start : 0,
                indent_level : 4,
                quote_keys   : false,
                space_colon  : false
        ...
        function indent(line) {
                if (line == null)
                        line = "";
                if (beautify)
                        line = $C.repeat_string(" ", beautify.indent_start + indentation * beautify.indent_level) + line;
                return line;
        };
        ...
        function with_indent(cont, incr) {
                if (incr == null) incr = 1;
                indentation += incr;
                var ret = cont.apply(null, $C.slice(arguments, 1));
                indentation -= incr;
                return ret;
        };
        ...
        var generators = {
                ...
                "binary": function(operator, lvalue, rvalue) {
                        ...
                        if ($C.member(lvalue[0], [ "assign", "conditional", "seq" ]) ||
                            lvalue[0] == "binary" && $C.PRECEDENCE[operator] > $C.PRECEDENCE[lvalue[1]]) {
                                left = "(" + left + ")";
                        }
                        if ($C.member(rvalue[0], [ "assign", "conditional", "seq" ]) ||
                            rvalue[0] == "binary" && $C.PRECEDENCE[operator] >= $C.PRECEDENCE[rvalue[1]]) {
                                right = "(" + right + ")";
                        }
                ...
                "unary-prefix": function(operator, expr) {
                        ...
                        if (!(HOP(DOT_CALL_NO_PARENS, expr[0]) || expr[0] == "num"))
                                val = "(" + val + ")";
                        return operator + ($C.is_alphanumeric_char(operator.charAt(0)) ? " " : "") + val;
                },
                ...
                "seq": function() {
                        return add_commas($C.slice(arguments).map(make));
                },
                "label": function(name, block) {
