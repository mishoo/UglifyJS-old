...
function ast_squeeze(ast) {
        ...
        function tighten(statements) {
                ...
                for (var i = 0, ret2 = [], prev, cur; i < ret1.length; ++i) {
                        cur = ret1[i];
        ...
        return w.with_walkers({
                ...
                "if": function(c, t, e) {
                        function maybe_swap(){
                                if (e && c[0] == "unary-prefix" && c[1] == "!") {
                                        // swap then/else branches and discard the negation
                                        ret = [ "if", c[2], e, t ];
                                }
                        };
                        c = walk(c);
...
function gen_code(ast, beautify) {
        ...
        function parenthesize(expr) {
                var gen = make(expr);
                return member(expr[0], slice(arguments, 1))
                        ? gen = "(" + gen + ")"
                        : gen;
        };
