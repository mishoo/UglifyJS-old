...
var HOP                  = $C.HOP;
var PRECEDENCE           = $C.PRECEDENCE;
var array_to_hash        = $C.array_to_hash;
var defaults             = $C.defaults;
var is_alphanumeric_char = $C.is_alphanumeric_char;
var is_identifier        = $C.is_identifier;
var member               = $C.member;
var repeat_string        = $C.repeat_string;
var slice                = $C.slice;
...
function ast_walker(ast) {
        ...
        var walkers = {
                ...
                "comment1": function(name) { return this },
                "comment2": function(name) { return this }
        };
        ...
        function walk(ast) {
                if (ast == null)
                        return null;
                // XXX: this helps us deal with generated names in macros.
                {
...
function ast_squeeze(ast, options) {
        ...
        function stats_to_sequences(statements) {
                for (var i = 0, ret = [], prev = null; i < statements.length; ++i) {
                        ...
                        if (!prev) {
                                ...
                                prev.push.apply(prev, slice(st, 1));
                        } else {
                                prev = null;
                                ret.push(st);
                        }
        ...
        function discard_noop_stats(statements) {
                for (var i = 0, ret = []; i < statements.length; ++i) {
                        var st = statements[i];
                        if (st[0] == "stat") {
                                switch (st[1][0]) {
...
function gen_code(ast, beautify) {
        ...
        function with_indent(cont, incr) {
                if (incr == null) incr = 1;
                indentation += incr;
                var ret = cont.apply(null, slice(arguments, 1));
                indentation -= incr;
                return ret;
        };
