...
function gen_code(ast, beautify) {
        ...
        var generators = {
                ...
                "binary": function(operator, lvalue, rvalue) {
                        ...
                        if (lvalue[0] == "binary" && PRECEDENCE[operator] > PRECEDENCE[lvalue[1]]) {
                                left = "(" + left + ")";
                        }
                        if (rvalue[0] == "binary" && PRECEDENCE[operator] > PRECEDENCE[rvalue[1]]) {
                                right = "(" + right + ")";
                        }
...
function process_ast(ast, options) {
        ...
        function define(name) {
                ...
                s[name] = true;
                if (options.mangle && (options.mangle_toplevel || s !== $toplevel)) {
        ...
        function next_mangled(n) {
                var cm = $current, next;
                if (cm.charAt(0) == "_") next = "_" + (parseInt(cm.substr(1), 10) + 1);
