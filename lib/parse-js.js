/***********************************************************************
  ...
  Exported functions:
    ...
    - process_ast(ast, options) -- process the AST and return a
      modified one.  It's able to compute simple (constant)
      expressions and rename variables to shorter names.
  ...
  The tokenizer/parser (the "meat" in this package) is a port to
  JavaScript of Marijn Haverbeke's "parse-js" Common Lisp library:
...
function gen_code(ast, beautify) {
        ...
        var generators = {
                ...
                "unary-prefix": function(operator, expr) {
                        var val = make(expr);
                        if (!(expr[0] in ATOMIC_START_TOKEN || expr[0] == "dot" || expr[0] == "call"))
                                val = "(" + val + ")";
...
function process_ast(ast, options) {
        ...
        function without_mangling(cont) {
                var mangle = options.mangle;
                options.mangle = false;
                try { return cont(); }
                finally { options.mangle = mangle; }
        };
        ...
        var walkers = {
                ...
                "function": function(name, args, body) {
                        ...
                        return with_new_scope(function(){
                                return [ "function", mname, args.map(define), body.map(walk) ];
                        });
                },
                ...
                "for-in": function(has_var, key, hash, block) {
                        key = has_var ? define(key) : mangled(key);
                ...
                "with": function(expr, block) {
                        return [ "with", walk(expr), without_mangling(function(){
                                return walk(block);
                        })];
                },
                "atom": function(name) {
