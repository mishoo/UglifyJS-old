...
function JS_Parse_Error(message, line, col, pos) {
        ...
        this.pos = pos;
        try {
                why.is.thi$.so.hard.to.get.right = parc;
        } catch(ex) {
                this.stack = ex.stack.substr(ex.stack.indexOf("\n") + 1);
        };
};
...
function tokenizer($TEXT, skip_comments) {
        ...
        function read_num(prefix) {
                var num = read_while(function(ch){
                        return is_alphanumeric_char(ch) || ch == "." || ch == "-";
                });
                if (prefix)
                        num = prefix + num;
                ...
                switch (S.token.type) {
                    ...
                    case "name":
                        return is_token(peek(), "punc", ":")
                                ? prog1(labeled_statement(S.token.value), next, next)
                                : simple_statement();
...
function generate_code(ast, beautify) {
        var indentation = 0;
        ...
        function addsp() {
                ...
                for (var i = 0; i < a.length; ++i) {
                        ...
                        b.push(a[i]);
                        if (next &&
                            is_alphanumeric_char(a[i].charAt(a[i].length - 1)) &&
                            is_alphanumeric_char(next.charAt(0))) {
                                b.push(" ");
                        }
                }
                return b.join("");
        ...
        function add_block_sc(lines) {
                for (var last = lines.length - 1, i = 0; i <= last; ++i) {
                        if ((beautify || i < last) && !/:$/.test(lines[i])) {
                                lines[i] += ";";
                        }
                }
                return lines;
        ...
        var generators = {
                ...
                "toplevel": function(statements) {
                        return add_block_sc(remove(statements.map(make), is_semicolon))
                                .map(indent)
                ...
                "throw": function(expr) {
                        return addsp("throw", make(expr));
                },
                "new": function(ctor, args) {
                        return addsp("new", make(ctor) + "(" + addcm(args.map(make)) + ")");
                },
                "switch": function(expr, body) {
                        return addsp("switch", "(" + make(expr) + ")", make_block(body));
                },
                "case": function(expr) {
                        return addsp("\x08case", make(expr) + ":");
                },
                "default": function() {
                        return "\x08default:";
                },
                ...
                "conditional": function(cond, the_then, the_else) {
                        return addsp(make(cond), "?", make(the_then), ":", make(the_else));
                },
                "assign": function(op, lvalue, rvalue) {
                        if (op && op !== true) op += "=";
                        else op = "=";
                        return addsp(make(lvalue), op, make(rvalue));
                },
                "dot": function(expr) {
                        var out = make(expr), i = 1;
                        if (expr[0] != "name" && expr[0] != "string" && expr[0] != "dot" && expr[0] != "sub")
                                out = "(" + out + ")";
                ...
                "call": function(func, args) {
                        switch (func[0]) {
                            case "function": // calling a literal function, need to put it into parens
                                return "(" + make(func) + ")(" + addcm(args.map(make)) + ")";
                            default:
                                return make(func) + "(" + addcm(args.map(make)) + ")";
                        }
                },
                ...
                "defun": function() {
                        return make_function.apply(this, arguments);
                },
                ...
                "for-in": function(has_var, key, hash, block) {
                        ...
                        if (has_var)
                                out += "var ";
                        out += addsp(make_name(key) + " in " + make(hash) + ")", make(block));
                        return out;
                },
                "while": function(condition, block) {
                        return addsp("while", "(" + make(condition) + ")", make(block));
                },
                "do": function(condition, block) {
                        return addsp("do", make(block), "while", "(" + make(condition) + ")");
                },
                "return": function(expr) {
                        return addsp("return", make(expr));
                },
                "binary": function(operator, lvalue, rvalue) {
                        return addsp(make(lvalue), operator, make(rvalue));
                },
                "unary-prefix": function(operator, expr) {
                        return operator + (is_alphanumeric_char(operator.charAt(0)) ? " " : "") + make(expr);
                },
                "unary-postfix": function(operator, expr) {
                        return make(expr) + operator;
                },
                ...
                "seq": function() {
                        return addcm(slice(arguments).map(make));
                }
        };
        ...
        function make_function(name, args, body) {
                ...
                out += "(" + addcm(args.map(make_name)) + ")";
                return addsp(out, make_block(body));
        };
        ...
        function make_1vardef(def) {
                ...
                if (val != null)
                        name = addsp(name, "=", make(val));
                return name;
        };

        function make_vardefs(defs) {
                return addcm(defs.map(make_1vardef));
        };

        function make_object(props) {
                if (props.length == 0)
                        return "{}";
                return addsp("{", addcm(props.map(function(p){
                        return addsp(make_string(p[0]), ":", make(p[1]));
                })), "}");
        };
        ...
        function make_array(elements) {
                if (elements.length == 0)
                        return "[]";
                return addsp("[", addcm(elements.map(make)), "]");
        };
...
exports.parse = parse;
exports.gen_code = generate_code;
