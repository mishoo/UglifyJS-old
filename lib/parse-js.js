...
function parse($TEXT, strict_mode) {
        ...
        function as() {
                return make_node
                        ? make_node(parser_helpers, obj_context, slice(arguments))
                        : slice(arguments);
        };
                ...
                switch (S.token.type) {
                    ...
                    case "keyword":
                        ...
                        if (CUSTOM_STATEMENT_PARSERS[kw])
                                return CUSTOM_STATEMENT_PARSERS[kw](parser_helpers, obj_context);
                        switch (kw) {
                            case "break":
        ...
        function subscripts(expr, allow_calls) {
                ...
                if (allow_calls && is("punc", "(")) {
                        if (expr[0] == "name" && CUSTOM_CALL_PARSERS[expr[1]]) {
                                return CUSTOM_CALL_PARSERS[expr[1]](parser_helpers, obj_context);
                        }
                        next();
